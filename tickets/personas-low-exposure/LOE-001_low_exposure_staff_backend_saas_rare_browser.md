# LOE-001 (P1) Persona: Ava Alvarez (`low_exposure_staff_backend_saas_rare_browser`)

- Status: [x] Done
- Output: `personas/low_exposure_staff_backend_saas_rare_browser.md`

## Goal
Create a **high-fidelity** persona document based on `PERSONA.md` (sections 0-10), in English.

## Seed (Must Match)
- Persona ID (snake_case): `low_exposure_staff_backend_saas_rare_browser`
- Display name: Ava Alvarez
- Archetype (1-3 words): Quiet Evaluator
- Role (short): Staff Backend Engineer at a B2B SaaS
- Budget range (monthly, can spend): $0–$200
- Skepticism level: `medium`
- Procurement reality: Self-serve for small tools; team approval and security review for prod-facing tooling
- Data they refuse to upload: Source code, production logs containing PII

## Behavioral Anchors
- Visits Product Hunt maybe once every 1–3 months (if at all); discovers tools via coworkers, internal Slack, or a trusted newsletter.
- Skips marketing copy; checks docs/API, integration steps, and failure modes.
- Almost never comments; if it looks useful they bookmark it and come back later. Bounce trigger: no clear pricing/CTA + no proof in the first minute.

## Deliverables
- Create `personas/low_exposure_staff_backend_saas_rare_browser.md`
- Fill **all** sections 0-10 from `PERSONA.md` with concrete detail (no placeholders)

## Acceptance Criteria
- `personas/low_exposure_staff_backend_saas_rare_browser.md` exists
- Includes sections 0-10 from `PERSONA.md` (fully filled)
- Voice block is 2nd person, 80-160 words
- Section 10 (Engine Mapping) is consistent with the rest of the persona

## Completion Notes
- Generated `personas/low_exposure_staff_backend_saas_rare_browser.md` via `scripts/generate-personas.mjs`
